...
class IndustryStockScreener:
    def __init__(self):
        ...
        self.excluded_columns = {
            ...
            'Change', 'Gap', 'Volume', 'Change from Open', 'Recom',  # Removed 'Avg Volume'
            'Rel Volume', 'Market Cap', 'P/E', 'Overall_Strength', 'Short_Term_Strength', 
            'Long_Term_Strength', 'SMA_Strength', 'Rank', 'Change From Open',
            # Removed 'Average Volume (3 Month)' 
            'Relative Volume', 'Market Cap.',
            ...
            'Industry_Name'  # Added as requested - duplicate of Industry
        }
        
        # Performance column renaming
        self.performance_column_mapping = {
            'Perf Week': 'W%',
            'Perf Month': 'M%',
            'Perf Quarter': 'Q%',
            'Perf Quart': 'Q%',  # Alternative naming
            'Perf Half': 'H%',
            'Perf Year': 'Y%',
            'Perf YTD': 'YTD%',
            'Performance (Week)': 'W%',
    ...
    def clean_and_process_dataframe(self, df: pd.DataFrame) -> pd.DataFrame:
        """Clean dataframe: remove excluded columns, rename performance columns, convert to numeric"""
        ...
        df = df.rename(columns=self.performance_column_mapping)
        
        # Convert performance columns to numeric (fix for Avg Monthly Performance issue)
        performance_cols = ['W%', 'M%', 'Q%', 'H%', 'Y%', 'YTD%']
        ...
        if 'Price' in df.columns and any(vol_col in df.columns for vol_col in ['Avg Volume', 'Average Volume (3 Month)']):
            # Determine which volume column to use
            volume_col = None
            if 'Avg Volume' in df.columns:
                volume_col = 'Avg Volume'
            elif 'Average Volume (3 Month)' in df.columns:
                volume_col = 'Average Volume (3 Month)'
            
            if volume_col:
                # Convert Price to numeric
                df['Price'] = pd.to_numeric(df['Price'], errors='coerce')
                
                # Convert volume to numeric (remove commas, handle K/M/B suffixes if present)
                volume_numeric = df[volume_col].astype(str).str.replace(',', '')
                # Handle K/M/B suffixes in volume data
                volume_numeric = volume_numeric.apply(self.parse_volume_with_suffix)
                
                # Calculate Average Dollar Volume
                avg_dollar_volume = df['Price'] * volume_numeric
                
                # Format with K/M/B suffixes
                df['Avg Dollar Volume'] = avg_dollar_volume.apply(self.format_dollar_volume)
                
                print(f"      💰 Added Average Dollar Volume column")
        
        # Reorder columns: Ticker, Company, Sector, Industry, Price, Avg Dollar Volume, then rest
        primary_columns = ['Ticker', 'Company', 'Sector', 'Industry', 'Price', 'Avg Dollar Volume']
        other_columns = [col for col in df.columns if col not in primary_columns]
    ...
    def parse_volume_with_suffix(self, volume_str: str) -> float:
        """Parse volume string that may contain K/M/B suffixes"""
        try:
            volume_str = str(volume_str).upper().strip()
            if volume_str in ['', 'NAN', 'NONE', '-']:
                return 0.0
            
            # Remove any non-numeric characters except K, M, B, and decimal point
            import re
            clean_str = re.sub(r'[^\d.KMB]', '', volume_str)
    ...
    def format_dollar_volume(self, value: float) -> str:
        """Format dollar volume with K/M/B suffixes"""
        try:
    ...
    def screen_industry_with_retries(self, industry_name: str) -> Optional[pd.DataFrame]:
        ...
        if tech_data is not None:
            tech_data = self.clean_and_process_dataframe(tech_data)
        ...
        if perf_filters:
            ...
            if perf_data is not None:
                perf_data = self.clean_and_process_dataframe(perf_data)
        ...
        if overview_filters:
            ...
            if overview_data is not None:
                overview_data = self.clean_and_process_dataframe(overview_data)
        ...
        for industry, data in self.industry_results.items():
            if data is not None and not data.empty:
                ...
                if 'M%' in data.columns:
                    top_stocks = data.nlargest(top_per_industry, 'M%')
                elif 'W%' in data.columns:
                    top_stocks = data.nlargest(top_per_industry, 'W%')
                else:
    ...
    def export_comprehensive_results(self, filename: str = None):
        ...
        try:
            with pd.ExcelWriter(filename, engine='openpyxl') as writer:
                ...
                for industry, data in self.industry_results.items():
                    if data is not None and not data.empty:
                        # Calculate average monthly performance (fixed)
                        avg_performance = 0
                        if 'M%' in data.columns:
                            # Make sure the column is numeric
                            monthly_perf = pd.to_numeric(data['M%'], errors='coerce')
                            avg_performance = monthly_perf.mean()
                        ...
                        summary_data.append({
                            ...
                            'Avg_Monthly_Performance': round(avg_performance, 2)
                        })
                        
                        # Add to all stocks (don't add Industry_Name as it's excluded)
